...
from scripts.summarize_focused_research import DEFAULT_FOCUSED_REGISTRY, build_parser
...
class FocusedResearchSummaryTests(unittest.TestCase):
    def write_registry(self, path: Path, rows: list[dict]):
        path.write_text("\n".join(json.dumps(row) for row in rows) + "\n", encoding="utf-8")

    def test_parser_defaults_to_focused_registry_and_top_15(self):
        args = build_parser().parse_args([])

        self.assertEqual(args.top_limit, 15)
        self.assertEqual(Path(args.registry), DEFAULT_FOCUSED_REGISTRY)

    def test_generates_focused_markdown_and_json(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.write_registry(registry, [
                ...
                registry_row("cfg2", classification="multi_window_reject", symbol="ETH", horizon=14, rr=2.5, atr=1.5),
            ])

            summary = summarize_registry(
                registry_path=registry,
                output_dir=tmp,
                filename_prefix="focused_v2a_global_summary",
                top_limit=15,
                refined=True,
            )
            markdown = Path(summary["markdown_path"]).read_text(encoding="utf-8")

        self.assertIn("focused_v2a_global_summary_", Path(summary["json_path"]).name)
